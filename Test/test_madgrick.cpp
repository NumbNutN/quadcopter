...
void TEST_Madgwick(void *arg) {
  ...
  for (;;) {
    ...
    // 为磁力计的误差函数计算梯度
    magnetic_err_func_gradient =
        magnetic_gradient(old_attitude, hmc_dev.getMagnetic().normalization());
    ...
    // 为四元数添加低通滤波去除尖刺
    // if(abs(quat_get_Pitch(old_attitude) - quat_get_Pitch((attitude))) > 0.2)
...
void TEST_Task_Print_Attitude(void *arg) {
  for (;;) {
    // if(quat_get_Pitch(attitude) > 3.1415926 || quat_get_Pitch(attitude) <
    // - 3.1415926)
    //     cout << 999 << endl;
    // cout << "M " << quat_get_Roll(attitude) << endl;
    // cout << "A " << quat_get_Roll(attitude -
    // 0.3*acceleration_err_fun_gradient.normalization()*deltaT) << endl;
