...
public:
  ...
  void setLabel(int8_t label) { _label = label; }

  void setDuty(float dutyCycle) {
    if (dutyCycle > MAX_CYCLE_DUTY)
      _dutyCycle = MAX_CYCLE_DUTY;
    else if (dutyCycle < MIN_CYCLE_DUTY)
      _dutyCycle = MIN_CYCLE_DUTY;
    else
      _dutyCycle = dutyCycle;
  }

  void setAddDuty(float deltaDutyCycle) {
    if (_dutyCycle + deltaDutyCycle > MAX_CYCLE_DUTY)
      _dutyCycle = MAX_CYCLE_DUTY;
    else if (_dutyCycle + deltaDutyCycle < MIN_CYCLE_DUTY)
      _dutyCycle = MIN_CYCLE_DUTY;
    else
  ...
  void setAngularVehicle(float omega) { setDuty(4.30148e-5 * omega + 0.05); }

  int8_t getLabel() const { return _label; }

  float get_dutyCycle() const { return _dutyCycle; }
};